...
Bit field defining whether Run Length Encoding has been applied to the data.  If set, the reverse transform will be applied using \textsc{DecodeRLE} after Order-0 or Order-1 uncompression (see Section~\ref{sec:ransRLE}).
...
The sum of these frequencies must be less than $2^{16}-16$ so after adjusting the frequencies it never go above the maximum unsigned 16-bit integer.
...
and some numeric, possibly with a leading zero.  We also observe some
...
\item{\textbf{DIGITS}:}
Fetch 4 bytes from the value byte stream and interpret these as a little
...
length from $B_{pos,DZLEN}$.  As per DIGITS, the value is interpreted as a
...
parameter is set.  The initial context value (reset per record) is
